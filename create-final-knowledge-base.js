...
// Read all the extracted data files
const qaData = JSON.parse(fs.readFileSync('extracted-questions-fixed.json', 'utf8')) || [];
const productData = JSON.parse(fs.readFileSync('website-data-backup.json', 'utf8')) || [];
const companyData = JSON.parse(fs.readFileSync('company-info-backup.json', 'utf8')) || [];

// Combine all data into one array with proper timestamps (newest first)
const allData = [
  ...companyData,      // Company facts
  ...productData,      // Product categories and URLs
  ...qaData           // Q&A pairs
].sort((a, b) => b.timestamp - a.timestamp);

// Generate the updated knowledgeBase.ts file content
const updatedKnowledgeBase = `import { KnowledgeItem } from '../types';
import * as db from './db';

// A knowledge base for Tallman Equipment - Updated ${new Date().toISOString().split('T')[0]}
...
export const retrieveContext = async (query: string): Promise<string[]> => {
  ...
  const queryWords = new Set(queryLower.split(/\s+/).filter(word => word.length > 2));
  ...
  const scoredDocuments = knowledgeBase.map(doc => {
    ...
    for (const word of queryWords) {
      if (docWords.has(word.toLowerCase())) {
        score += 3; // Increased score for exact matches
        console.log(\`  ✓ Exact match: "\${word}" (+3)\`);
      }
    }

    // Partial matches (substring search) - improved
    for (const word of queryWords) {
      const wordLower = word.toLowerCase();
      if (lowerContent.includes(wordLower)) {
        score += 2; // Increased score for partial matches
        console.log(\`  ✓ Partial match: "\${wordLower}" (+2)\`);
      }

      // Also check for common variations (stringer vs stringing)
      if (wordLower === 'stringer' && lowerContent.includes('stringing')) {
        score += 2;
        console.log(\`  ✓ Stringer->Stringing variation (+2)\`);
      }
      if (wordLower === 'stringing' && lowerContent.includes('stringer')) {
        score += 2;
        console.log(\`  ✓ Stringing->Stringer variation (+2)\`);
      }
      if (wordLower.includes('string') && lowerContent.includes('stringing')) {
        score += 1;
        console.log(\`  ✓ String contained in stringing (+1)\`);
      }
    }

    // Boost score for location-related keywords
    const locationKeywords = ['location', 'located', 'address', 'headquarters', 'hq', 'office', 'warehouse', 'columbus', 'indiana', 'florida', 'where', 'drive', 'dr', 'street', 'st', 'website', 'online', 'nationwide', 'usa', 'branches', 'addison', 'lake', 'city', 'tallman'];
    ...
    // Extra boost for direct location queries
    if (queryLower.includes('where') || queryLower.includes('location') || queryLower.includes('located')) {
      if (lowerContent.includes('hq:') || lowerContent.includes('branches:') || lowerContent.includes('columbus') || lowerContent.includes('addison') || lowerContent.includes('lake city')) {
        score += 30;
      }
    }
  ...
  // Return the top 5 most relevant documents if they have a score > 0
...
const filesExist = [
  'extracted-questions-fixed.json',
  'website-data-backup.json',
  'company-info-backup.json'
].every(file => fs.existsSync(file));

if (!filesExist) {
  console.error('❌ Required backup files not found. Please run the extraction scripts first.');
  console.error('Missing files:', filesExist.map((exists, i) => exists ? null : filesExist[i]).filter(Boolean));
  process.exit(1);
}

// Write the updated knowledgeBase.ts file
fs.writeFileSync('services/knowledgeBase.ts', updatedKnowledgeBase);

console.log('✅ Knowledge base file updated successfully!');
console.log(`📊 Added ${qaData.length} Q&A items, ${productData.length} product categories, and ${companyData.length} company facts`);
console.log(`📊 Total items in DEFAULT_KNOWLEDGE_BASE: ${allData.length}`);
console.log('📝 File: services/knowledgeBase.ts');
