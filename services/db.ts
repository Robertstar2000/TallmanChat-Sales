...
export const getAllChatHistories = async (username: string): Promise<ChatHistoryItem[]> => {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        const transaction = db.transaction([CHAT_HISTORY_STORE], 'readonly');
        const store = transaction.objectStore(CHAT_HISTORY_STORE);
        ...
        request.onsuccess = () => {
            // Only return id and title for the history list, filtered by username
            const allHistories: ChatSession[] = request.result;
            const userHistories = allHistories.filter(session => session.username === username);
            // Sort by ID descending (which will be timestamp based)
            userHistories.sort((a, b) => b.id.localeCompare(a.id));
            resolve(userHistories.map(({ id, title }) => ({ id, title })));
        };
        request.onerror = () => {
...
export const clearKnowledgeBase = async (): Promise<void> => {
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => resolve();
        request.onerror = () => {
            console.error('Error clearing knowledge base:', request.error);
            reject(request.error);
        };
    });
};
...
export const setState = async (key: string, value: any): Promise<void> => {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => resolve();
      request.onerror = () => {
          console.error(`Error setting state for key '${key}':`, request.error);
          reject(request.error);
      };
    });
};

export const getState = async (key: string): Promise<any | undefined> => {
    const db = await openDB();
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => resolve(request.result);
        request.onerror = () => {
            console.error(`Error getting state for key '${key}':`, request.error);
            reject(request.error);
        };
    });
};
